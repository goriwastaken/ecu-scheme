...
/**
 * @brief Computes the mesh width of a triangular mesh which is defined as the
 * maximal edge length
...
double EvaluateMeshFunction(std::shared_ptr<const lf::mesh::Mesh> mesh_p, MF mf,
  ...
  for (const lf::mesh::Entity* e : mesh_p->Entities(0)) {
    LF_ASSERT_MSG(lf::base::RefEl::kTria() == e->RefEl(),
    ...
    if (local_coords(0) >= 0.0 - tol && local_coords(1) >= 0.0 - tol &&
        local_coords(0) + local_coords(1) <= 1.0 + tol) {
...
/**
 * @brief Evaluates a MeshFunction at a set of points specified by their global
 * coordinates
...
void SampleMeshFunctionOnCurve(
    std::shared_ptr<const lf::mesh::Mesh> mesh_p, const CURVE& gamma,
    const MF& mf, unsigned int num_points, const std::string& file_name,
    std::pair<double, double> interval = {0.0, 1.0}) {
  // Uniform sample along gamma
  Eigen::VectorXd tau =
      Eigen::VectorXd::LinSpaced(num_points, interval.first, interval.second);
  ...
  for (int i = 0; i < num_points; ++i) {
    file << tau(i) << ", " << sample_points[i](0) << ", " << sample_points[i](1)
         << ", " << result[i] << std::endl;
  }
  file.close();
}
