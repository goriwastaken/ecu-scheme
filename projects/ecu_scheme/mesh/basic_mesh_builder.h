 ...
 *
 * Some experiments discussed in the thesis require a simple tensor product mesh
 * on the unit square This class builds such a mesh with variable number of
...
class BasicMeshBuilder {
 public:
  ...
  /**
   * @brief Builds a tensor product mesh with an arbitrary dimension of the
   * square
   ...
   * @return a shared pointer to the mesh
   */
  std::shared_ptr<lf::mesh::Mesh> Build(double topLeftCornerX,
