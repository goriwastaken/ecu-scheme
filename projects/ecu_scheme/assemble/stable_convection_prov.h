...
lf::mesh::utils::CodimMeshDataSet<double> initMassesVert(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

lf::mesh::utils::CodimMeshDataSet<double> initMassesEdges(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

lf::mesh::utils::CodimMeshDataSet<double> initMassesCells(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);
...
class StableConvectionUpwindMatrixProvider {
 public:
  StableConvectionUpwindMatrixProvider(const std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>> &fe_space,
                                       FUNCTOR v,
...
template <typename SCALAR, typename FUNCTOR>
StableConvectionUpwindMatrixProvider<SCALAR, FUNCTOR>::StableConvectionUpwindMatrixProvider(const std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>> &fe_space, FUNCTOR v, lf::mesh::utils::CodimMeshDataSet<double> masses_vertices, lf::mesh::utils::CodimMeshDataSet<double> masses_edges, lf::mesh::utils::CodimMeshDataSet<double> masses_cells)
: fe_space_(fe_space), v_(v), masses_vertices_(masses_vertices), masses_edges_(masses_edges), masses_cells_(masses_cells) {}


template <typename SCALAR, typename FUNCTOR>
Eigen::Matrix<SCALAR, 6, 6> StableConvectionUpwindMatrixProvider<SCALAR, FUNCTOR>::Eval(const lf::mesh::Entity &entity) {
  ...
  Eigen::Vector2d test_bary = (corners.col(0) + corners.col(1) + corners.col(2)) / 3.0;
...
//        LF_ASSERT_MSG(element_matrix(l, 0) == 0, "Element matrix entry must be zero");
    ...
    Eigen::Matrix<double, 1, 6> barycenter_row_contribution = masses_cells_(entity) * (v_(barycenter).transpose() * gradientsLocalShapeFunctions(barycenter)).cwiseProduct(localShapeFunctions(barycenter));
    element_matrix.row(l) += barycenter_row_contribution;

  }
//  Eigen::Matrix<double, 6, 6> barycenter_contribution = masses_cells_(entity) * (v_(barycenter).transpose() * gradientsLocalShapeFunctions(barycenter)).transpose() * localShapeFunctions(barycenter);
//  element_matrix += barycenter_contribution;
