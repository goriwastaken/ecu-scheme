...
/**
 * @brief Computes the masses m(p) of all vertices of the mesh for the linear
...
/**
 * @brief Computes the masses m(p) of all vertices of the mesh for the quadratic
 * finite element space
 * @param mesh_p pointer to the mesh
 * @return data structure containing the masses m(p) for all vertices of the
 * mesh represented by mesh_p
...
/**
 * @brief Computes the masses m(p) of all edge midpoints of the mesh for the
 * quadratic finite element space
 * @param mesh_p pointer to the mesh
 * @return data structure containing the masses m(p) for all edge midpoints of
 * the mesh represented by mesh_p
 */
...
class ConvectionUpwindMatrixProvider {
 public:
  ...
  ConvectionUpwindMatrixProvider(
      ...
      lf::mesh::utils::CodimMeshDataSet<double> masses);
  ...
  [[nodiscard]] bool isActive(const lf::mesh::Entity& /*entity*/) const {
    return true;
  }

 private:
  FUNCTOR v_;  // velocity field
  std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>>
      fe_space_;  // finite element space
  lf::mesh::utils::CodimMeshDataSet<double>
      masses_vertices_;  // masses of all vertex nodes of the mesh
  lf::mesh::utils::CodimMeshDataSet<double>
      masses_edges_;  // masses of all edge midpoints of the mesh
};
...
ConvectionUpwindMatrixProvider<SCALAR, FUNCTOR>::Eval(
    const lf::mesh::Entity& entity) {
  ...
  LF_ASSERT_MSG(num_local_dofs == 3 || num_local_dofs == 6,
                "Class supports only linear and quadratic FE spaces");
  ...
  if (num_local_dofs == 3) {
    ...
    LF_ASSERT_MSG(local_masses.size() == 3,
                  "There must be three masses, one for each basis function");
    // the gradients in the linear case are obtained as the vertex normals, just
    // with opposite sign
    Eigen::MatrixXd gradients_linearFE = -vertex_normals;

    for (int l = 0; l < 3; ++l) {
      Eigen::Vector2d vl = velocities.col(l);
      // Vertex a^l is upwind iff product of v(a^l) with both adjacent normals
      // is positive
      if (vl.dot(vertex_normals.col((l + 2) % 3)) >= 0 &&
          vl.dot(vertex_normals.col((l + 1) % 3)) >= 0) {
        // a^l is upwind
        element_matrix.row(l) =
            local_masses[l] * vl.transpose() * gradients_linearFE;
      } else {
        // a^l is not upwind
    ...
    // std::cout << "local masses: " << local_masses[0] << ", " <<
    // local_masses[1] << ", " << local_masses[2] << ", " << local_masses[3] <<
    // ", " << local_masses[4] << ", " << local_masses[5] << std::endl;
    ...
    // Matrix of gradients of local shape functions based on global coordinates
    // of nodes in quadratic Lagrangian FE space
    ...
    for (int l = 0; l < 6; ++l) {
      ...
      if (l < 3) {
        ...
        if (vl.dot(vertex_normals.col((l + 2) % 3)) >= 0 &&
          ...
          LF_ASSERT_MSG(element_matrix.row(l).norm() == 0,
                        "Vertex masses should be 0 in this case");
        } else {
          // a^l is not upwind
        ...
        const Eigen::Matrix<double, 2, 3> outward_normals =
            computeOutwardNormalsTria(entity);
        // Midpoint m^l is upwind iff product of v(m^l) with corresponding
        // outward normals is positive
