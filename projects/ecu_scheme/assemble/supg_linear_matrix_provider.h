...
void InflowBC(const std::shared_ptr<lf::uscalfe::UniformScalarFESpace<double>>& fe_space,
  ...
  /**
   * @brief Computes the local matrices for the convective term based on linear FE and trapezoidal rule
   ...
   * @return a 3x3 matrix containing the element matrix.
   ...
   template <typename FUNCTOR>
   Eigen::Matrix3d SupgConvectiveEMP<FUNCTOR>::Eval(const lf::mesh::Entity& entity){
  ...
  Eigen::Matrix3d SupgStabilizationEMP<FUNCTOR_EPS, FUNCTOR_V, FUNCTOR_F>::Eval(
    ...
    const double delta = ecu_scheme::assemble::Delta(entity, eps_(corners.col(0)), v_(corners.col(0)));
  ...
  Eigen::VectorXd SolveCDBVPSupg(
    ...
    // Stabilization term:
    ecu_scheme::assemble::SupgStabilizationEMP<decltype(eps), decltype(v), decltype(f)> stabilization_provider(eps, v, f);
