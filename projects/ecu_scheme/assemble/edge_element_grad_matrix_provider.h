...
/**
 * @brief Class providing the edge element gradient matrix for a given FE space and velocity field
...
class EdgeElementGradMatrixProvider{
 public:
  EdgeElementGradMatrixProvider(const std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>>& fe_space, FUNCTOR v);
...
template <typename SCALAR, typename FUNCTOR>
Eigen::Matrix<SCALAR, Eigen::Dynamic, Eigen::Dynamic> EdgeElementGradMatrixProvider<SCALAR, FUNCTOR>::Eval(const lf::mesh::Entity& entity) const {
  ...
  if(num_local_dofs == 3){
    ...
    // compute part of the element matrix -- what is given by \int_T \nabla \lambda_i \psi_{e_j}
    const Eigen::Matrix3d contribution_element_matrix = (area/3.0) * edge_element.transpose() * grads;
    ...
    auto edge_elem_basis = [edge_orientations, bary_functions, bary_functions_grad](const Eigen::Vector2d& x){
      ...
      edge_elem_basis.col(0) = basis_lambda_grad.col(1) * basis_lambda.col(0) - basis_lambda_grad.col(0) * basis_lambda.col(1);
      edge_elem_basis.col(1) = basis_lambda_grad.col(2) * basis_lambda.col(1) - basis_lambda_grad.col(1) * basis_lambda.col(2);
      edge_elem_basis.col(2) = basis_lambda_grad.col(0) * basis_lambda.col(2) - basis_lambda_grad.col(2) * basis_lambda.col(0);
    ...
    // account for contribution that is row-wise constant -- (u \cdot psi_{e_i})(x_i)
    // contribution is 0 iff the vertex x_i is not part of the upwind triangle
    for(int i = 0; i < 3; ++i){
      Eigen::Vector2d velocity_at_vertex = v_(corners.col(i));
      // compute contribution
      double contribution = velocity_at_vertex.transpose() * edge_elem_basis((Eigen::Vector2d() << corners(0, i), corners(1, i)).finished()).col(i);

      if(velocity_at_vertex.dot(vertex_normals.col((i+2)%3)) >= 0 && velocity_at_vertex.dot(vertex_normals.col((i+1)%3)) >= 0){
