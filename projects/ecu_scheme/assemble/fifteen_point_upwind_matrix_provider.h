 ...
 * @return a vector of pairs, where each pair contains a quadrature point and its weight
 */
std::vector<std::tuple<Eigen::Vector2d, double, bool>> prepareFifteenPointQuadRule();

lf::mesh::utils::CodimMeshDataSet<double> initMassesVerticesFifteenQuadRule(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

lf::mesh::utils::CodimMeshDataSet<double> initMassesEdgeMidpointsFifteenQuadRule(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

lf::mesh::utils::CodimMeshDataSet<double> initMassesEdgeOffFifteenQuadRule(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

lf::mesh::utils::CodimMeshDataSet<double> initMassesCellsFifteenQuadRule(const std::shared_ptr<const lf::mesh::Mesh>& mesh_p);

template <typename SCALAR, typename FUNCTOR>
class FifteenPointUpwindMatrixProvider {
 public:
  FifteenPointUpwindMatrixProvider(const std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>> &fe_space,
                                   FUNCTOR v,
...
template <typename SCALAR, typename FUNCTOR>
FifteenPointUpwindMatrixProvider<SCALAR, FUNCTOR>::FifteenPointUpwindMatrixProvider(const std::shared_ptr<lf::fe::ScalarFESpace<SCALAR>> &fe_space, FUNCTOR v, lf::mesh::utils::CodimMeshDataSet<double> masses_vertices, lf::mesh::utils::CodimMeshDataSet<double> masses_edge_midpoints, lf::mesh::utils::CodimMeshDataSet<double> masses_edges, lf::mesh::utils::CodimMeshDataSet<double> masses_cells)
: fe_space_(fe_space), v_(v), masses_vertices_(masses_vertices), masses_edge_midpoints_(masses_edge_midpoints), masses_edges_(masses_edges), masses_cells_(masses_cells) {}

template <typename SCALAR, typename FUNCTOR>
Eigen::Matrix<SCALAR, 6, 6> FifteenPointUpwindMatrixProvider<SCALAR, FUNCTOR>::Eval(const lf::mesh::Entity &entity) {
  ...
  Eigen::Matrix<SCALAR, 6, 6> element_matrix = Eigen::Matrix<SCALAR, 6, 6>::Zero();
  ...
  for(int i = 0; i <= kMaxDegreeOfExactness; ++i){
    for(int j = 0; j <= kMaxDegreeOfExactness; ++j){
      ...
      quadrature_points.col(temp_index) = (Eigen::Vector2d(2, 1) << ((double)i / (double)kMaxDegreeOfExactness), ((double)j / (double)kMaxDegreeOfExactness)).finished();
      temp_index++;
    }
  }
  LF_ASSERT_MSG(temp_index == 15, "There must be 15 quadrature points");
  // In order to compare the obtained points we instantiate the precomputed Quadrature rule for the reference triangle
  const std::vector<std::tuple<Eigen::Vector2d, double, bool>> quad_rule_reference = prepareFifteenPointQuadRule();
  ...
  // Prepare a vector to store the 9 different quad points not available in S_2^0(M)

  std::vector<Eigen::Vector2d> truncated_transformed_quad_points;
  // Transform the quadrature points from reference coords to physical coordinates
  Eigen::MatrixXd transformed_quadrature_points = geo_ptr->Global(quadrature_points);
  // Remove the quadrature points that are associated with vertices and edge midpoints
  // We are interested only in the other 9 quad points
  Eigen::MatrixXd edge_off_points(2, 6);
  edge_off_points << transformed_quadrature_points.col(1), transformed_quadrature_points.col(3), transformed_quadrature_points.col(5),
      transformed_quadrature_points.col(8), transformed_quadrature_points.col(12), transformed_quadrature_points.col(13);
  Eigen::MatrixXd cell_center_points(2, 3);
  cell_center_points << transformed_quadrature_points.col(6), transformed_quadrature_points.col(7), transformed_quadrature_points.col(10);
  ...
  auto localShapeFunctions = [bary_functions](const Eigen::Vector2d xh) -> Eigen::Matrix<double, 1, 6>{
    ...
    shapeFunctions << temp(0,0) * (2 * temp(0,0) - 1), temp(0,1) * (2 * temp(0,1) - 1), temp(0,2) * (2 * temp(0,2) - 1),
        4 * temp(0,0) * temp(0,1), 4 * temp(0,1) * temp(0,2), 4 * temp(0,0) * temp(0,2);
  ...
  for(int l = 0; l < 6; ++l){
    ...
    if(l < 3){
      ...
      if(vl.dot(vertex_normals.col((l+2)%3)) >= 0 && vl.dot(vertex_normals.col((l+1)%3)) >= 0){
        ...
        LF_ASSERT_MSG(element_matrix(l, 0) == 0, "Element matrix entry must be zero");
      }else{
        // a^l is not upwind
      ...
      const Eigen::Matrix<double, 2, 3> outward_normals = ecu_scheme::assemble::computeOutwardNormalsTria(entity);
    ...
    // Make a case distinction since LehrFEM++ edge numbering is a bit different than the numbering from how the quad points are created
    int temp_local_index_edges = 0;
    for(auto off_edge : edge_off_points.colwise()){
      Eigen::Matrix<double, 1, 6> off_edge_contribution = (v_(off_edge).transpose() * gradientsLocalShapeFunctions(off_edge)).cwiseProduct(localShapeFunctions(off_edge));
      // add upwind property because of edges
      const Eigen::Matrix<double, 2, 3> outward_normals_off = ecu_scheme::assemble::computeOutwardNormalsTria(entity);
...
//      }else{
        if(temp_local_index_edges == 0 || temp_local_index_edges == 1){
          ...
          std::cerr << "There should be a total of 6 edge points that are not edge midpoints" << std::endl;
        }
//      }
    ...
    for(auto cell_node : cell_center_points.colwise()){
      Eigen::Matrix<double, 1, 6> cell_center_contribution = masses_cells_(entity) * (v_(cell_node).transpose() * gradientsLocalShapeFunctions(cell_node)).cwiseProduct(localShapeFunctions(cell_node));
