...
Eigen::Matrix<double, 2, 3> computeTangentialComponents(const lf::mesh::Entity &entity);
void EnforceInflowBDCOneform(
    ...
    std::function<Eigen::Matrix<double, 2, 1>(const Eigen::Matrix<double, 2, 1, 0> &)> dirichlet);
/**
...
//    lf::assemble::COOMatrix<double> A(N_dofs, N_dofs);
    // Our problem involves mixed function spaces, namely for lagrangian FE and one-form edge element basis functions
    // We need to account for the different DOFs  involved in the Nedelec FE space
    const bool predicate_fe_space = fe_space_->LocGlobMap().NumLocalDofs(*(mesh_p->EntityByIndex(0, 0))) == 3;
    const lf::assemble::DofHandler &dofh_edge = predicate_fe_space ? lf::assemble::UniformFEDofHandler(mesh_p, {{lf::base::RefEl::kSegment(), 1}}) : lf::assemble::UniformFEDofHandler(mesh_p, {{lf::base::RefEl::kSegment(), 2}, {lf::base::RefEl::kTria(), 2}});
    const lf::assemble::size_type N_dofs_edge = dofh_edge.NumDofs();
    // Sizes of matrices are in the order: Nr. DOFs trial space, Nr. DOFs test space
    ...
    // Full system matrix based on Thesis chapter: Implementation for assembly of 1-form experiment matrices
    lf::assemble::COOMatrix<double> A_full_galerkin(N_dofs + N_dofs_edge, N_dofs + N_dofs_edge);
    ...
    if (fe_space_->LocGlobMap().NumLocalDofs(*(mesh_p->EntityByIndex(0, 0))) ==
      ...
      lf::assemble::AssembleMatrixLocally(0, dofh_edge, dofh_edge,
                                          edge_element_mass_matrix_provider, A_debug);
      ...
      lf::assemble::AssembleMatrixLocally(0, dofh_edge, dofh_edge,
                                          edge_element_grad_matrix_provider, A_debug);
    }

    Eigen::VectorXd phi(N_dofs_edge);
    phi.setZero();

//    // Assemble the full system matrix based on the sub-matrices obtained previously
//    // Iterate over triplets and add up entries to full Galerkin matrix
//    const std::vector<Eigen::Triplet<double>> triplets_edge_bary = A_edge_bary.triplets();
//    const std::vector<Eigen::Triplet<double>> triplets_edge_edge = A_edge_edge.triplets();
