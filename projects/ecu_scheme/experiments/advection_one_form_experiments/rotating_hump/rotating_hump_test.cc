...
int main(int argc, char* argv[]) {
  ...
  // setup is \Omega = [-1, 1]^2 and time interval I = [0, T=1] with time
  // dependent Lipschitz continuous velocity field u(x, t) = sin(2* \pi *
  // t)(1-x^2)(1-y^2) * [y, -x] initial condition w_0 defined as bump, see
  // Thesis chapter Numerical Experiments
  ...
  //  const auto kVelocityField = [](const Eigen::Vector3d &xh) ->
  //  Eigen::Vector2d{
  //    // xh has components (x, y, t), where t represents the time component
  //    return (std::sin(2.0 * M_PI * xh(2)) * (1.0 - xh(0) * xh(0)) * (1.0 -
  //    xh(1) * xh(1))) * (Eigen::Vector2d() << xh(1), -xh(0)).finished();
  //  };
  //
