...
Eigen::Matrix<double, 2, 3> computeTangentialComponents(const lf::mesh::Entity &entity){
  const Eigen::Matrix<double, 2, 3> tria_normals = computeOutwardNormals(entity);
...
void EnforceInflowBDCOneform(
  ...
  const lf::assemble::DofHandler &dofh_edge = lf::assemble::UniformFEDofHandler(mesh_p, {{lf::base::RefEl::kSegment(), 1}});
  ...
  // remember initEssentialConditionFromFunction() doesn't work if underlying FE space remains the lagrangian one
//  auto selector = [&](lf::base::size_type idx) -> std::pair<bool, double> {
//    const auto &e = dofh.Entity(idx);
//    return {e.RefEl() == lf::base::RefElType::kSegment() && bd_flags(*e), 0};
//  };

  std::vector<std::pair<bool, double>> ess_dof_select(dofh_edge.NumDofs(), {false, 0.0});
  ...
  // This allows to precompute the outward normals, and thus tangential components beforehand
  for(const lf::mesh::Entity *entity: mesh_p->Entities(0)){
    const Eigen::Matrix<double, 2, 3> tangential_components = computeTangentialComponents(*entity);
    ...
    for(const lf::mesh::Entity *edge : entity->SubEntities(1)){
      ...
      if(bd_flags(*edge)){
        const Eigen::Vector2d eval_bdc_at_midpoint = dirichlet((endpoints.col(0) + endpoints.col(1)) / 2.);
        const double tg_contribution = tangential_components.col(edge_idx).transpose() * eval_bdc_at_midpoint;

        auto gdof_indices{dofh_edge.GlobalDofIndices(*edge)};
        LF_ASSERT_MSG(gdof_indices.size() == 1, "Edge should have 1 DOF for edge element basis functions");
