...
int main(int argc, char* argv[]){
  ...
  if(argc == 3){
    ...
    if(eps_for_refinement > 0.0){
      std::cerr << "This experiment concerns the pure transport problem" << std::endl;
    }
  }
...
//    }
//    return -M_PI*x(0)*x(1)*std::cos(0.5*M_PI*(x(1)*x(1) + 1.0))*std::sin(0.5*M_PI*(x(1)*x(1) + 1.0));
//  };
//
...
//  };
  ...
  Eigen::VectorXd exact_test_vector = lf::fe::NodalProjection(*fe_space_linear, mf_test_exact_plot);
  lf::fe::MeshFunctionFE mf_test_exact_linear(fe_space_linear, exact_test_vector);
  ecu_scheme::post_processing::output_meshfunction_paraview<double, decltype(mf_test_exact_linear)>(
      fe_space_linear, mf_test_exact_linear, "concentric_stream_solution_test_exact");

  Eigen::VectorXd exact_test_vector_quad = lf::fe::NodalProjection(*fe_space_quad, mf_test_exact_plot);
  ecu_scheme::post_processing::output_results<double>(
      fe_space_quad, exact_test_vector_quad, "concentric_stream_solution_test_exact_quad");

//  lf::mesh::utils::MeshFunctionGlobal mf_test_exact{test_exact};
//  Eigen::VectorXd exact_test_vector = lf::fe::NodalProjection(*fe_space_linear, mf_u_exact);
//
//  ecu_scheme::post_processing::output_results<double>(
//      fe_space_linear, exact_test_vector, "concentric_stream_solution");

  // Compute the L2 error norms with regular refinement
  std::shared_ptr<lf::refinement::MeshHierarchy> mesh_hierarchy_p = lf::refinement::GenerateMeshHierarchyByUniformRefinemnt(mesh_p, refinement_levels);
  ...
  ecu_scheme::post_processing::ExperimentSolutionWrapper<double> solution_collection_wrapper_quad{
      refinement_levels, eps_for_refinement, Eigen::VectorXd::Zero(refinement_levels), mesh_hierarchy_p, std::vector<Eigen::VectorXd>{}
  };
  ...
  ecu_scheme::post_processing::ExperimentSolutionWrapper<double> solution_collection_wrapper_supg_linear{
      refinement_levels, eps_for_refinement, Eigen::VectorXd::Zero(refinement_levels), mesh_hierarchy_p, std::vector<Eigen::VectorXd>{}
  };
  ...
  ecu_scheme::post_processing::ExperimentSolutionWrapper<double> solution_collection_wrapper_quad_stable{
      refinement_levels, eps_for_refinement, Eigen::VectorXd::Zero(refinement_levels), mesh_hierarchy_p, std::vector<Eigen::VectorXd>{}
  };

  ecu_scheme::post_processing::ExperimentSolutionWrapper<double> solution_collection_wrapper_quad_fifteen_upwind{
      refinement_levels, eps_for_refinement, Eigen::VectorXd::Zero(refinement_levels), mesh_hierarchy_p, std::vector<Eigen::VectorXd>{}
  };
  ...
  for(int l = 0;l < L; ++l){
    ...
    ecu_scheme::experiments::ConcentricStreamSolution<double> experiment_l(fe_space_l);
    //todo debug
//    Eigen::VectorXd solution_vector_l = experiment_l.ComputeSolution( kEps, kVelocity, kDirichletFunctor);
//    solution_collection_wrapper_quad.final_time_solutions.push_back(solution_vector_l);
    // midpoint upwind scheme
//    Eigen::VectorXd solution_vector_l = experiment_l.ComputeSolutionMultipleMethods(kEps, kVelocity, kDirichletFunctor, kTestFunctor, "UPWIND");
//    solution_collection_wrapper_quad.final_time_solutions.push_back(solution_vector_l);
    // 7-point scheme from HH08
    Eigen::VectorXd solution_vector_l_stable_upwind = experiment_l.ComputeSolutionMultipleMethods(kEps, kVelocity, kDirichletFunctor, kTestFunctor, "STABLE_UPWIND");
    solution_collection_wrapper_quad_stable.final_time_solutions.push_back(solution_vector_l_stable_upwind);
    // 15 point upwind scheme
//    Eigen::VectorXd solution_vector_l_fifteen_pt_upwind = experiment_l.ComputeSolutionMultipleMethods(kEps, kVelocity, kDirichletFunctor, kTestFunctor, "15P_UPWIND");
//    solution_collection_wrapper_quad_fifteen_upwind.final_time_solutions.push_back(solution_vector_l_fifteen_pt_upwind);
    ...
    Eigen::VectorXd solution_vector_supg_quad = ecu_scheme::assemble::SolveCDBVPSupgQuad<decltype(kTempEps), decltype(kVelocity), decltype(kTestFunctor), decltype(kDirichletFunctor)>(fe_space_l, kTempEps, kVelocity, kTestFunctor, kDirichletFunctor);
...
//    ecu_scheme::experiments::ConcentricStreamSolution<double> experiment_l_linear(fe_space_l_linear);
//    Eigen::VectorXd solution_vector_l_linear = experiment_l_linear.ComputeSolution( kEps, kVelocity, kDirichletFunctor);
//    solution_collection_wrapper_linear.final_time_solutions.push_back(solution_vector_l_linear);


    ecu_scheme::experiments::ConcentricStreamSolution<double> experiment_supg_l_linear(fe_space_l_linear);
...
//  ecu_scheme::post_processing::convergence_comparison_toSUPG<double, decltype(mf_u_exact)>(
//      solution_collection_wrapper_linear, solution_collection_wrapper_supg_linear,
//      mf_u_exact,
//      ecu_scheme::post_processing::concat(
//          "concentric_stream_solution_comparison_linear", "_", refinement_levels, "_",
//          eps_for_refinement),
//      true
//      );
...
//  ecu_scheme::post_processing::convergence_comparison_toSUPG<double, decltype(mf_u_exact)>(
//      solution_collection_wrapper_quad, solution_collection_wrapper_supg_quad,
//      mf_u_exact,
//      ecu_scheme::post_processing::concat(
//          "concentric_stream_solution_comparison_quad", "_", refinement_levels, "_",
//          eps_for_refinement)
//  );
...
//      {solution_collection_wrapper_quad, "UPWIND"},
  ...
  ecu_scheme::post_processing::convergence_comparison_multiple_methods<double, decltype(mf_u_exact)>(
      bundle_solution_wrappers,
      mf_u_exact,
      ecu_scheme::post_processing::concat(
          "concentric_stream_solution_multiple_methods_quad", "_", refinement_levels, "_",
          eps_for_refinement)
      );
  return 0;
}