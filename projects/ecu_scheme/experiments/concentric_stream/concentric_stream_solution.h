...
/**
 * @brief Flags all nodes on the inflow boundary - basically a different implementation to do the same thing as EnforceBoundaryConditions
...
class ConcentricStreamSolution{
 public:
  ...
  explicit ConcentricStreamSolution(const std::shared_ptr< lf::uscalfe::UniformScalarFESpace<SCALAR>>& fe_space)
      : fe_space_(fe_space) {}
  ...
  Eigen::Vector<double, Eigen::Dynamic> ComputeSolution(double eps,
    ...
    lf::uscalfe::ReactionDiffusionElementMatrixProvider laplacian_provider(
        fe_space_, lf::mesh::utils::MeshFunctionConstant(0.0), lf::mesh::utils::MeshFunctionConstant(0.0)
    );
    ...
    if(fe_space_->LocGlobMap().NumLocalDofs(*(mesh_p->EntityByIndex(0,0))) == 6){
      ...
      ecu_scheme::assemble::ConvectionUpwindMatrixProvider<SCALAR, decltype(velocity)> convection_provider(fe_space_, velocity, ecu_scheme::assemble::initializeMasses(mesh_p), ecu_scheme::assemble::initializeMassesQuadraticEdges(mesh_p));
      lf::assemble::AssembleMatrixLocally(0, dofh, dofh, convection_provider, A);
    }
...
//        A, phi);
    ...
    Eigen::VectorXd dirichlet_coefficients = lf::fe::NodalProjection(*fe_space_, mf_g_dirichlet);
    lf::assemble::FixFlaggedSolutionCompAlt<double>(
        [&inflow_nodes, &dirichlet_coefficients, &dofh](lf::assemble::glb_idx_t dof_idx) -> std::pair<bool, double>{
...
//    EnforceBoundaryConditions(fe_space_, A, phi, dirichlet);
   ...
   * @param velocity
   * @param dirichlet
   * @param method_name should be either {"UPWIND", "STABLE_UPWIND", "15P_UPWIND"}
   * @return
   */
  Eigen::Vector<double, Eigen::Dynamic> ComputeSolutionMultipleMethods(double eps,
                                                                       std::function<Eigen::Matrix<double, 2, 1, 0>(const Eigen::Matrix<double, 2, 1, 0> &)> velocity,
                                                                       std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> dirichlet,
                                                                       std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> test_functor,
    ...
    lf::mesh::utils::MeshFunctionGlobal<decltype(velocity)> mf_velocity{velocity};
    lf::mesh::utils::MeshFunctionGlobal<decltype(dirichlet)> mf_g_dirichlet{dirichlet};
    lf::mesh::utils::MeshFunctionGlobal<decltype(test_functor)> mf_f_test{test_functor};
...
//    lf::assemble::AssembleMatrixLocally(0, dofh, dofh, laplacian_provider, A);
    ...
    if(fe_space_->LocGlobMap().NumLocalDofs(*(mesh_p->EntityByIndex(0,0))) == 6){
      if(method_name == "UPWIND"){
        ...
        ecu_scheme::assemble::StableConvectionUpwindMatrixProvider<SCALAR, decltype(velocity)> stable_convection_provider(fe_space_, velocity, ecu_scheme::assemble::initMassesVert(mesh_p), ecu_scheme::assemble::initMassesEdges(mesh_p), ecu_scheme::assemble::initMassesCells(mesh_p));
        lf::assemble::AssembleMatrixLocally(0, dofh, dofh, stable_convection_provider, A);
      }else if(method_name == "15P_UPWIND"){
        ecu_scheme::assemble::FifteenPointUpwindMatrixProvider<SCALAR, decltype(velocity)> fifteen_point_upwind_matrix_provider(fe_space_, velocity, ecu_scheme::assemble::initMassesVerticesFifteenQuadRule(mesh_p),
                                                                                                                                ecu_scheme::assemble::initMassesEdgeMidpointsFifteenQuadRule(mesh_p),
                                                                                                                                ecu_scheme::assemble::initMassesEdgeOffFifteenQuadRule(mesh_p),
                                                                                                                                ecu_scheme::assemble::initMassesCellsFifteenQuadRule(mesh_p));
        lf::assemble::AssembleMatrixLocally(0, dofh, dofh, fifteen_point_upwind_matrix_provider, A);
      }else{
    ...
    phi.setZero();
    // Assemble RHS
    lf::uscalfe::ScalarLoadElementVectorProvider elvec_provider(fe_space_, mf_f_test);
...
////      Eigen::MatrixXd Sigma = Eigen::MatrixXd::Zero(A_svd.rows(), A_svd.cols());
////      const unsigned int p = sv.size();
////      Sigma.block(0,0,p,p) = sv.asDiagonal();
//      // U, Sigma, V tuple
//      const unsigned int last_col_sing_vector = V.cols() - 1;
//      Eigen::VectorXd smallest_singular_vector = V.col(last_col_sing_vector);
...
//      lf::io::VtkWriter vtk_writer(fe_space_->Mesh(), PROJECT_BUILD_DIR "/results/concentric_stream_singular_vector.vtk", 0, 2);
