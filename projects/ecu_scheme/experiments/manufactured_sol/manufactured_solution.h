...
//      EnforceBoundaryConditions(fe_space_, A, phi, dirichlet);
    ...
    Eigen::Vector<double, Eigen::Dynamic> ComputeSUPGSolution(double eps,
      ...
      ecu_scheme::assemble::SUPGElementMatrixProvider supg_elmat_provider( mf_velocity, true);
...
//      EnforceBoundaryConditions(fe_space_, A, phi, dirichlet);
     ...
     * @return
     */
    Eigen::Vector<double, Eigen::Dynamic> ComputeStableSolution(double eps,
                                                                std::function<Eigen::Matrix<double, 2, 1, 0>(const Eigen::Matrix<double,2,1,0>&)> velocity,
                                                                std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> test,
                                                                std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> dirichlet
    ) const {
      ...
      ecu_scheme::assemble::StableConvectionUpwindMatrixProvider<SCALAR, decltype(velocity)> stable_elmat_provider(fe_space_, velocity, ecu_scheme::assemble::initMassesVert(mesh_p), ecu_scheme::assemble::initMassesEdges(mesh_p), ecu_scheme::assemble::initMassesCells(mesh_p));
      lf::assemble::AssembleMatrixLocally(0, dofh, dofh, stable_elmat_provider, A);
...
//      EnforceBoundaryConditions(fe_space_, A, phi, dirichlet);
    ...
    Eigen::Vector<double, Eigen::Dynamic> ComputeFifteenPointQuadRuleSolution(double eps,
                                                                              std::function<Eigen::Matrix<double, 2, 1, 0>(const Eigen::Matrix<double,2,1,0>&)> velocity,
                                                                              std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> test,
                                                                              std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> dirichlet
        ) const {
      ...
      // Convective part - using the Stable scheme b^{2b, vmb} from HH08
      ecu_scheme::assemble::FifteenPointUpwindMatrixProvider<SCALAR, decltype(velocity)> fifteen_point_elmat_provider(fe_space_, velocity, ecu_scheme::assemble::initMassesVerticesFifteenQuadRule(mesh_p),
      ...
      lf::assemble::AssembleMatrixLocally(0, dofh, dofh, fifteen_point_elmat_provider, A);
