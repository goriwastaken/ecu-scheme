 ...
 */
//Eigen::VectorXd step(const Eigen::VectorXd& u0_vector, double tau, Eigen::SparseMatrix<double> convection_matrix_sparse);
...
class ConstVeloSolution {
 public:
  ...
  explicit ConstVeloSolution(const std::shared_ptr< lf::uscalfe::UniformScalarFESpace<SCALAR>>& fe_space)
      : fe_space_(fe_space) {}

  Eigen::VectorXd ComputeSolution(std::function<double(const Eigen::Matrix<double, 2, 1, 0> &)> dirichlet,
                                               std::function<Eigen::Matrix<double, 2, 1, 0>(const Eigen::Matrix<double,2,1,0>&)> velocity,
                                               std::function<double(const Eigen::Matrix<double, 2, 1, 0>&)> test_function,
    ...
    if(method_name == "UPWIND") {
      ...
      ecu_scheme::assemble::StableConvectionUpwindMatrixProvider<double,
                                                                   decltype(velocity)>
    ...
    phi.setZero();
    lf::fe::ScalarLoadElementVectorProvider<double, decltype(mf_test_function)> load_provider(fe_space_, mf_test_function);
